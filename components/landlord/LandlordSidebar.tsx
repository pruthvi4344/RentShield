      ...
      <div className="px-6 py-5 border-b border-slate-100">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">Landlord Portal</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {navItems.map((item) => {
          ...
          const badge =
            ...
            item.id === "requests" ? pendingRequests :
            undefined;
          return (
            <button
              key={item.id}
            ...
            >
              ...
              {badge && badge > 0 && (
                <span className="flex-shrink-0 min-w-[20px] h-5 rounded-full bg-teal-500 text-white text-xs font-bold flex items-center justify-center px-1.5">
                  {badge}
                </span>
              )}
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-white border-b border-slate-100 px-4 py-3 flex items-center justify-between shadow-sm">
        ...
        <button onClick={() => setMobileOpen(!mobileOpen)} className="p-2 rounded-lg text-slate-600 hover:bg-slate-100 transition-colors">
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-30 flex">
          <div className="w-72 bg-white h-full shadow-2xl flex flex-col mt-[57px]">{sidebarContent}</div>
